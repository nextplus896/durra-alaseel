    ...
    @foreach ($kyc_fields as $item)
        @if ($item->type == 'select')
            <div class="col-xl-3 col-lg-3 col-md-12 mb-20">
                <label class="title" for="{{ $item->name }}">{{ $item->label }} <span>@if ($item->validation->required)*@else {{ __('(Optional)') }}@endif</span></label>
        ...
        @elseif ($item->type == 'file')
            <div class="col-xl-4 col-lg-4 col-md-6 mb-20 form-group">
                <label>{{ __($item->label) }} <span>@if ($item->validation->required)* @else {{ __('(Optional)') }} @endif</span></label>
