...
@section('content')
    ...
    <div class="custom-card mt-15">
        <div class="card-header">
            ...
            <span class="{{ $user->kycStringStatus->class }}">{{ __($user->kycStringStatus->value) }}</span>
            @include('admin.components.link.custom',[
                'href'          => setRoute('admin.vendor.details',$user->username),
                'text'          => __("Profile"),
                'class'         => "btn btn--base",
                ...
                <div class="product-sales-btn">
                    @if ($user->kyc_verified != global_const()::VERIFIED)
                        @include('admin.components.button.custom',[
                            ...
                            'text'          => __("Approve"),
                            'permission'    => "admin.vendor.kyc.approve",
                        ])
                    @endif
                    ...
                    @if ($user->kyc_verified != global_const()::REJECTED)
                        @include('admin.components.button.custom',[
                            ...
                            'text'          => __("Reject"),
                            'permission'    => "admin.vendor.kyc.reject",
                        ])
                    @endif
                </div>
            @else
...
@push('script')
    <script>
        $(".approve-btn").click(function(){
            var actionRoute = "{{ setRoute('admin.vendor.kyc.approve',$user->username) }}";
