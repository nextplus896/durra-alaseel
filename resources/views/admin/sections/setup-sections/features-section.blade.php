...
@section('content')
                                ...
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    @foreach ($languages as $item)
                                        <button class="nav-link @if (get_default_language_code() == $item->code) active @endif"
                                            ...
                                            aria-controls="{{ $item->code }}" aria-selected="true">{{ $item->name }}
                                        </button>
                                    @endforeach
                            ...
                            <div class="tab-content" id="nav-tabContent">
                                @foreach ($languages as $item)
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif"
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                'label' => __('Details Title'),
                                                'label_after' => '*',
                                                'placeholder' => __('Write Here') . '...',
                                                'name' => $item->code . '_details_title',
                                                'value' => old(
                                                    $item->code . '_details_title',
                                                    $data->value->language->$lang_code->details_title ?? ''),
                                            ])
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                'label' => __('Details Button'),
                                                'label_after' => '*',
                                                'placeholder' => __('Write Here') . '...',
                                                'name' => $item->code . '_details_button',
                                                'value' => old(
                                                    $item->code . '_details_button',
                                                    $data->value->language->$lang_code->details_button ?? ''),
                                            ])
                                        </div>
                                    </div>
                                @endforeach
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#features-item-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i>
                        {{ __('Add Item') }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                <td><span
                                        class="text--dark">{{ $item->language->$system_default_lang->feature ?? '' }}</span>
                                </td>
                                <td>
    ...
    @include('admin.components.modals.site-section.add-feature-page-item')
                                ...
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    @foreach ($languages as $item)
                                        <button class="nav-link @if (get_default_language_code() == $item->code) active @endif"
                                            ...
                                            data-bs-target="#edit-modal-{{ $item->code }}" type="button" role="tab"
                                            aria-controls="edit-modal-{{ $item->code }}"
                                    ...
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif"
                                        ...
                                        <div class="form-group">
                                            @include('admin.components.form.input', [
                                                ...
                                                'label_after' => '*',
                                                'placeholder' => __('Write Here') . '...',
                                                'name' => $lang_code . '_feature_edit',
                                                'value' => old(
                                                    $lang_code . '_feature_edit',
                                                    $data->value->language->$lang_code->feature ?? ''),
                                            ])
                                        </div>
                                    </div>
                                @endforeach
    ...
    <script>
        ...
        $(".edit-modal-button").click(function() {
            ...
            $.each(languages, function(index, item) {
                editModal.find("input[name=" + item.code + "_feature_edit]").val(oldData.language[item.code]
                    ?.feature);
            });
