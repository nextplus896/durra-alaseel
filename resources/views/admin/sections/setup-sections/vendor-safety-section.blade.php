...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#vendor-safety-item-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Item") }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td><span class="text--dark">{{ textLength($item->language->$system_default_lang->description ?? "", 35) }}</span></td>
                                <td>
    ...
    @include('admin.components.modals.site-section.add-vendor-safety-item')

    {{-- edit modal --}}
    <div id="vendor-safety-item-edit" class="mfp-hide large">
                    ...
                    <div class="row mb-10-none mt-3">
                        <div class="language-tab">
                            <nav>
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    @foreach ($languages as $item)
                                        <button class="nav-link @if (get_default_language_code() == $item->code) active @endif" id="edit-modal-{{$item->code}}-tab" data-bs-toggle="tab" data-bs-target="#edit-modal-{{$item->code}}" type="button" role="tab" aria-controls="edit-modal-{{ $item->code }}" aria-selected="true">{{ $item->name }}</button>
                                    @endforeach
                                </div>
                            </nav>
                            <div class="tab-content" id="nav-tabContent">
                                ...
                                @foreach ($languages as $item)
                                    @php
                                        $lang_code = $item->code;
                                    @endphp
                                    <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="edit-modal-{{ $item->code }}" role="tabpanel" aria-labelledby="edit-modal-{{$item->code}}-tab">
                                        <div class="form-group">
                                            @include('admin.components.form.input',[
                                                'label'     => __("Title"),
                                                'label_after'   => '*',
...
@push('script')
    ...
    <script>
        openModalWhenError("vendor-safety-item-add","#vendor-safety-item-add");
        openModalWhenError("vendor-safety-item-edit","#vendor-safety-item-edit");
        ...
        $(".edit-modal-button").click(function(){
            ...
            var editModal = $("#vendor-safety-item-edit");
            ...
            openModalBySelector("#vendor-safety-item-edit");
