        ...
        <div class="card-body">
            <form class="card-form">
                <div class="row align-items-center mb-10-none">
                        ...
                        <ul class="user-profile-list-two">
                            ...
                            <li class="three">{{ __('Full Name') }}: <span>
                                    @if ($data->user_id != null)
                                        <a href="{{ setRoute('admin.users.details', $data->user->username) }}">{{ $data->user->fullname }}
                                            ({{ __('USER') }})</a>
                                    @endif
                                </span></li>
                            <li class="four">{{ __('Method') }}: <span>{{ @$data->gateway_currency->name }}</span></li>
                            <li class="five">{{ __('Amount') }}:
                                <span>{{ get_amount(@$data->request_amount, $data->request_currency) }}</span>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="col-xl-4 col-lg-4 form-group">
                        <ul class="user-profile-list two">
                            <li class="one">{{ __('Fees & Charges') }}:
                                <span>{{ number_format(@$data->total_charge, 2) }}
                                    {{ @$data->payment_currency }}</span>
                            </li>
                            <li class="two">{{ __('Conversion Amount') }}:
                                <span>{{ number_format(@$data->request_amount * $data->exchange_rate, 2) }}
                                    {{ @$data->payment_currency }}</span>
                            </li>
                            <li class="three">{{ __('Rate') }}: <span>1 {{ $data->request_currency }} =
                                    {{ number_format(@$data->exchange_rate, 2) }}
                                    {{ @$data->payment_currency }}</span></li>
                            <li class="four">{{ __('Total Payable') }}:
                                <span>{{ number_format(@$data->total_payable, 2) }}
                                    {{ @$data->payment_currency }}</span>
                            </li>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header p-3" id="approvedModalLabel">
                        <h5 class="modal-title">{{ __('Approved Confirmation') }} ( <span
                                class="fw-bold text-danger">{{ get_amount(@$data->request_amount, $data->request_currency) }}</span>
                            )</h5>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header p-3" id="rejectModalLabel">
                        <h5 class="modal-title">{{ __('Rejection Confirmation') }} ( <span
                                class="fw-bold text-danger">{{ get_amount(@$data->request_amount, $data->request_currency) }}</span>
                            )</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
