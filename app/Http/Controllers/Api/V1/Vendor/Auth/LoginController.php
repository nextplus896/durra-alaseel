...
class LoginController extends Controller
{
    ...
    */
    ...
    public function login(Request $request)
    {
        ...
        if(!Vendor::where($this->username(),$validated['credentials'])->exists()) {
            return Response::error([__("Vendor doesn't exists!")],[],404);
        }

        $user = Vendor::where($this->username(),$validated['credentials'])->first();
        if(!$user) return Response::error([__("Vendor doesn't exists!")]);
    ...
    protected function authenticated(Request $request, $user, $token)
    {
        ...
        return Response::success([__('Vendor successfully logged in')],[
            'token'         => $token,
            'user_info'     => $user->only([
                'id',
                'firstname',
