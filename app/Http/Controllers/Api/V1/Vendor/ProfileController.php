...
class ProfileController extends Controller
{
    ...
    public function profileInfoUpdate(Request $request) {
        ...
        if(Vendor::whereNot('id',$user->id)->where("full_mobile",$validated['full_mobile'])->exists()) {
    ...
    public function profilePasswordUpdate(Request $request) {
        ...
        if(!Hash::check($validated['current_password'],auth()->guard("vendor_api")->user()->password)) {
            return Response::error([__("Current password didn't match")],[],400);
        }

        try{
            auth()->guard("vendor_api")->user()->update([
    ...
    public function deleteProfile(Request $request){
        ...
        return Response::success([__('Your account deleted successfully!')]);
    }
