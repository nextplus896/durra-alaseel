...
class AppOnboardScreensController extends Controller
{
     ...
     * @return view
     */
    public function vendorOnboardScreens() {
        $page_title = __("Vendor Onboard Screen");
        $onboard_screens = VendorAppOnboardScreen::orderByDesc('id')->get();
        return view('admin.sections.app-settings.vendor-onboard-screens',compact(
            'page_title',
            'onboard_screens',
        ));
    }
     ...
     * @param closer
     */
    public function vendorOnboardScreenStore(Request $request) {
        $validator = Validator::make($request->all(),[
            'image'     => 'required|image|mimes:png,jpg,webp,svg,jpeg',
            'title'     => 'nullable|string|max:120',
            'sub_title' => 'nullable|string|max:255',
        ...
        try{
            VendorAppOnboardScreen::create($validated);
        }catch(Exception $e) {
            return back()->with(['error' => [__('Something went wrong! Please try again.')]]);
        }
     ...
     * Function for update onboard screen status by AJUX request
     */
    public function vendorOnboardScreenStatusUpdate(Request $request) {
        $validator = Validator::make($request->all(),[
            'data_target'   => 'required|numeric',
            'status'        => 'required|numeric',
            'input_name'    => 'required|string',
        ...
        $onboard_screen = VendorAppOnboardScreen::find($target_id);
        if(!$onboard_screen) {
            $error = ['error' => [__('Onboard screen not found!')]];
            return Response::error($error,null,404);
        }
     ...
     * Function for update specific onboard screen information
     */
    public function vendorOnboardScreenUpdate(Request $request) {
        $target = $request->target ?? "";
        $onboard_screen = VendorAppOnboardScreen::find($target);
        if(!$onboard_screen) {
            return back()->withErrors($request->all())->withInput()->with(['warning' => [__('Onboard screen not found!')]]);
        }
        $request->merge(['old_image' => $onboard_screen->image]);
     ...
     * @param  \Illuminate\Http\Request  $request
     */
    public function vendorOnboardScreenDelete(Request $request) {
        $validator = Validator::make($request->all(),[
            'target'        => 'required|integer|exists:vendor_app_onboard_screens,id',
        ]);
        $validated = $validator->validate();

        try{
            VendorAppOnboardScreen::find($validated['target'])->delete();
        }catch(Exception $e){
            return back()->with(['error' => [__('Something went wrong! Please try again.')]]);
        }
