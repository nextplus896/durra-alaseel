...
class AppSettingsController extends Controller
{
     ...
     * @return view
     */
    public function vendorSplashScreen() {
        $page_title = __("Vendor Splash Screen");
        $app_settings = AppSettings::first();
        return view('admin.sections.app-settings.vendor-splash-screen',compact(
            'page_title',
            'app_settings',
        ));
    }
    ...
    public function vendorSplashScreenUpdate(Request $request) {
        $validator = Validator::make($request->all(),[
            'image'         => 'nullable|image|mimes:png,jpg,jpeg,webp,svg',
            'vendor_version'       => 'required|string|max:15',
        ]);
        $validated = $validator->validate();
        $validated = Arr::except($validated,['image']);
        ...
        if($request->hasFile('image')) {
            $image = get_files_from_fileholder($request,'image');
            $upload_image = upload_files_from_path_static($image,'app-images',$app_settings->vendor_splash_screen_image,true,true);
            $validated['vendor_splash_screen_image']   = $upload_image;
        }
     ...
     * @return view
     */
    public function vendorUrls() {
        $page_title = __("Vendor App URLs");
        $app_settings = AppSettings::first();
        return view('admin.sections.app-settings.vendor-urls',compact(
            'page_title',
            'app_settings',
        ));
    }
    ...
    public function vendorUrlsUpdate(Request $request) {
        $validator = Validator::make($request->all(),[
            'vendor_android_url'   => 'required|string|url|max:255',
            'vendor_iso_url'       => 'nullable|string|url|max:255|different:vendor_android_url',
        ]);
        $validated = $validator->validate();
