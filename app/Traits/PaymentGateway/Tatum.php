            ...
            if(request()->expectsJson()) { // making for API response
                return [
                    ...
                    'address_info'      => [
                        ...
                        'submit_url'    => route('api.user.car.booking.payment.crypto.confirm',$trx_id)
                    ],
                ];
            }

            return redirect()->route('user.car.booking.payment.crypto.address', $trx_id);
        }
    ...
    public function tatumSubscriptionForAccountTransaction($crypto_asset, $address) {
        ...
        $response = Http::withHeaders([
            ...
            'attr'  => [
                ...
                'url'       => route('user.car.booking.payment.callback',['gateway' => $gateway->alias,'token' => PaymentGatewayConst::CALLBACK_HANDLE_INTERNAL])
            ]
        ])->throw(function(Response $response, RequestException $exception) {
            throw new Exception($exception->getMessage());
        })->object();
