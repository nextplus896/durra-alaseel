...
trait CoinGate
{
    ...
    public function coingateCallbackResponse($reference, $callback_data, $output = null)
    {
        ...
        if (isset($output['transaction']) && $output['transaction'] != null && $output['transaction']->status != PaymentGatewayConst::STATUSSUCCESS) {
            // if transaction already created & status is not success
            ...
            if ($callback_status == $this->coinGate_status_paid) {
                ...
                try {
                    DB::table($output['transaction']->getTable())
                        ->where('id', $output['transaction']->id)
                        ->update([
                    ...
                    $transaction = Transaction::where('id', $output['transaction']->id)->first();

                    $wallet = $transaction->bookings->cars->vendor->wallets;

                    if ($transaction->bookings->status == CarBookingConst::STATUSCOMPLETE) {
                        if ($wallet->due_payment != 0 && $wallet->due_payment == $transaction->receive_amount) {
                            $charge = $wallet->due_payment;
                            $this->insertProfit($charge);
                            $wallet->update([
                                'due_payment' => 0,
                            ]);
                        } elseif ($wallet->due_payment != 0 && $wallet->due_payment < $transaction->receive_amount) {
                            $charge = $wallet->due_payment;
                            $this->insertProfit($charge);
                            $wallet->update([
                                'balance' => $wallet->balance + ($transaction->receive_amount - $wallet->due_payment),
                                'due_payment' => 0,
                            ]);
                        } elseif ($wallet->due_payment != 0 && $wallet->due_payment > $transaction->receive_amount) {
                            $charge = $transaction->receive_amount;
                            $this->insertProfit($charge);
                            $wallet->update([
                                'due_payment' => $wallet->due_payment - $transaction->receive_amount,
                            ]);
                        } elseif ($wallet->due_payment == 0) {
                            $wallet->update([
                                'balance' => $wallet->balance + $transaction->receive_amount,
                            ]);
                        }
                    }
            ...
            // need to create transaction and update status if needed
            ...
            $this->insertForCallBack($output, $status);
        }
